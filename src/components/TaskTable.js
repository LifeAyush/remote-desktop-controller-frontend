...
const TaskTable = ({ 
  ...
  const getStatusColor = (status) => {
    ...
    if (statusLower.includes('exited') || statusLower.includes('dead')) return 'error';
              ...
              <React.Fragment key={task.id}>
                ...
                >
                  <TableCell padding="checkbox">
                    ...
                    >
                      {expandedRow === task.id ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                    </IconButton>
                  </TableCell>
                  ...
                  <TableRow className={darkMode ? 'bg-gray-900' : 'bg-gray-50'}>
                    <TableCell colSpan={8} className="p-4">
                      <Box className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <h4 className="text-sm font-semibold mb-2">Container Details</h4>
                          <div className="text-xs space-y-1">
                            <p><span className="font-medium">ID:</span> {task.id}</p>
                            <p><span className="font-medium">Created:</span> {new Date(task.created).toLocaleString()}</p>
                            <p><span className="font-medium">Ports:</span> {task.ports || 'None'}</p>
                            <p><span className="font-medium">Network:</span> {task.network || 'default'}</p>
                          </div>
                        </div>
                        <div>
